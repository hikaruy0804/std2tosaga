...
class Std2saga:
    def __init__(self, model_name_or_path="sonoisa/t5-base-japanese", model_dir="model", max_input_length=64, max_target_length=64, csv_path='方言辞書.csv'): #train_batch_size=64, eval_batch_size=64, num_train_epochs=20):
...
#     def unicode_normalize(self, cls, s): #軽量化のためコメントアウト
...
#     def remove_extra_spaces(self, s): #軽量化のためコメントアウト
#         s = re.sub('[ 　]+', ' ', s)
...
#     def normalize_neologd(self, s):#軽量化のためコメントアウト
#         s = s.strip()
...
#         return s
    ...
    def normalize_text(self, text):
        ...
        text = text.replace("\t", " ")
        text = text.strip()
        #text = self.normalize_neologd(text) #軽量化のためコメントアウト
        text = text.lower()
        return text
